...
int vrpn_Generic_Server_Object::setup_BPack(char * & pch, char * line, FILE * config_file)
{
	char name [LINESIZE], device [LINESIZE];
	int baud_rate, period, avgCnts;

	next();
	// Get the arguments (class, BPack_name, port, baud, period, average counters)
	if (sscanf(pch,"%511s%511s%d%d%d",name,device, &baud_rate, &period, &avgCnts) != 5)
	{
		fprintf(stderr,"Bad vrpn_BPack line: %s\n",line);
		return -1;
	}
	// Make sure there's room for a new analog
	if (num_analogs >= VRPN_GSO_MAX_ANALOG) {
	...
	// Open the device
	if (verbose) {
		printf("Opening vrpn_BPack: %s on port %s, baud %d\n",
			name,device,baud_rate);
	}
	if ((analogs[num_analogs] =
		new vrpn_BPack (name, connection, device, baud_rate, period, avgCnts)) == NULL) {
			fprintf(stderr,"Can't create new vrpn_BPack\n");
			return -1;
	} else {
		num_analogs++;
	}
